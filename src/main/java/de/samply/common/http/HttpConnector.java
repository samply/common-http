...
/**
 * Connector class to provide apache http connectors and jersey http connectors
 * includes http and https proxy support.
 */
public class HttpConnector {
    ...
    public final static String PROXY_BYPASS_PRIVATE_NETWORKS = "proxy.bypass.private.networks";
    ...
    public HttpConnector(HashMap<String, String> config) {
        ...
        try {
            bypassProxyForPrivateNetworks = Boolean.parseBoolean(config.get(PROXY_BYPASS_PRIVATE_NETWORKS));
        } catch (Exception e) {
    ...
    /**
     * Initializes the clients If no https proxy is configured, both clients are
     * the same.
     */
    ...
    /**
     * Returns the result of a request submitted as ClosableHttpResponse NB: Do
     * not forget to close the Response when done with it.
     *
     ...
     * @param params                        HashMap{@literal <}String,String{@literal >} of params to be added to the URL
     * @param mediaType                     Content-type of the request (defaults to application/json)
    ...
    public CloseableHttpResponse doAction(String action, String URL,
        ...
        return doAction(action, URL, headers, params, mediaType, request, preemptiveAuth, preemptiveProxyAuthentication, 0);
    }
    ...
    /**
     * Returns the result of a request submitted as a HashMap with the fields:
     * body: The Response body as String statuscode: The Response statuscode as
     * int headers: The Response headers as Header[].
     *
     * @param action          action method as string: get, post
     * @param URL             The target URL
     * @param headers         HashMap{@literal <}String,String{@literal >} of headers to be added
    ...
    public HashMap<String, Object> doActionHashMap(String action, String URL,
        ...
        return doActionHashMap(action, URL, headers, params, contentType, request, preemptiveAuth, preemptiveProxy, 0);
    }
    ...
    /**
     * Transforms a apache httpclient to a jersey client also enables POJO
     * MAPPING as feature.
     *
     * @param httpClient              the http client
     * @param failOnUnknownProperties if the client shall fail on deserialisation on unknown or unignorable properties
    ...
    /**
     * @see HttpConnector#getClient(CloseableHttpClient, Boolean)
     * This client will fail on unknown properties.
    ...
    public HttpHost getProxy(String target) throws MalformedURLException {
        ...
        return (targetUrl.getProtocol().equalsIgnoreCase(PROTOCOL_HTTPS)) ? getHttpsProxy() : getHttpProxy();
    }
    ...
    public CloseableHttpClient getHttpClient(HttpHost target) {
        ...
        return (target.getSchemeName().startsWith(PROTOCOL_HTTPS)) ? getHttpClientForHTTPS() : getHttpClientForHTTP();
    }
        ...
        if (proxy != null) {
            routePlanner = new DefaultProxyRoutePlanner(proxy) {
                ...
                public HttpRoute determineRoute(final HttpHost host,
                    ...
                    if (isLoopbackAddress(hostname) || (bypassProxyForPrivateNetworks && isLocalAddress(hostname))) {
                        // Return direct route
    ...
    /**
     * Check if a given IPv4 Address in its string representation belongs to a private network (according to RFC 1918).
     * <p>
     * Use to check if proxies should be bypassed.
     *
     * @param ipAddress the IP address to check as a String
     * @return true if it belongs to a private network
     * false otherwise
     */
